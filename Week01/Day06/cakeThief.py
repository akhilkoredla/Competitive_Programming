...
def  max_duffel_bag_value(cake_tuples,capacity):
    ...
    #Dynamic programming
    solved = {} # a dict to save solutions space = capacity
    for c in range(0,capacity+1):
        maxvalue = 0
        for cake in cake_tuples:
            rest = c - cake[0]
            if rest >= 0: # if fit in
                value = cake[1] + solved[rest]
                if value >  maxvalue:
                    maxvalue = value # find the best combo
        solved[c] = maxvalue # save the best solution of capcity c
    return maxvalue
